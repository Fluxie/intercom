...
impl Foo {
    ...
    fn struct_method(&self, input: PlainStruct ) -> PlainStruct {
        PlainStruct { field: 0 }
    }
...
#[allow(dead_code)]
#[doc(hidden)]
unsafe extern "C" fn __Foo_Foo_Automation_struct_method_Automation(
    self_vtable: ::intercom::RawComPtr,
    input: PlainStruct,
) -> PlainStruct {
    let result: Result<PlainStruct, ::intercom::ComError> = (|| {
        let self_combox = (self_vtable as usize - __Foo_Foo_AutomationVtbl_offset())
            as *mut ::intercom::ComBox<Foo>;
        let self_struct: &Foo = &**self_combox;
        let __result = self_struct.struct_method(input.into());
        Ok({ __result.into() })
    })();
    use intercom::ErrorValue;
    match result {
        Ok(v) => v,
        Err(err) => <PlainStruct as ErrorValue>::from_error(::intercom::return_hresult(err)),
    }
}
#[allow(non_snake_case)]
#[allow(dead_code)]
...
#[allow(dead_code)]
#[doc(hidden)]
unsafe extern "C" fn __Foo_Foo_Raw_struct_method_Raw(
    self_vtable: ::intercom::RawComPtr,
    input: PlainStruct,
) -> PlainStruct {
    let result: Result<PlainStruct, ::intercom::ComError> = (|| {
        let self_combox =
            (self_vtable as usize - __Foo_Foo_RawVtbl_offset()) as *mut ::intercom::ComBox<Foo>;
        let self_struct: &Foo = &**self_combox;
        let __result = self_struct.struct_method(input.into());
        Ok({ __result.into() })
    })();
